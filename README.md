...
- **You want a slim per-turn token footprint.** In our [published eval](bench/), `zb`'s **total surface tax** — the catalog + skill bytes the agent carries every turn — came in ~94% below the captured Zoho MCP catalog by the same metric. Both surfaces are tunable, so audit the methodology and re-run against your own configuration.
